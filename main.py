...
from models import Samsung as SamsungModel
...
class BaseMethod():
    ...
    def __init__(self):
        self.raw_weight = {'kamera': 2, 'baterai': 4, 'layar': 4, 'ram': 3, 'processor': 5, 'harga': 5}
    ...
    @property
    def data(self):
        query = select(SamsungModel.no, SamsungModel.type, SamsungModel.kamera, SamsungModel.baterai, SamsungModel.layar,
                       SamsungModel.ram, SamsungModel.processor, SamsungModel.harga)
        result = session.execute(query).fetchall()
        print(result)
        return [{'no': Samsung.no, 'type': Samsung.type, 'kamera': Samsung.kamera, 'baterai': Samsung.baterai,
                'layar': Samsung.layar, 'ram': Samsung.ram, 'processor': Samsung.processor, 'harga': Samsung.harga} for Samsung in result]
...
class WeightedProduct(Resource):
    ...
    def post(self):
        ...
        result = calculator.calculate
        return {'samsung': sorted(result, key=lambda x: x['score'], reverse=True)}, HTTPStatus.OK.value
...
class Samsung(Resource):
    ...
    def get(self):
        query = session.query(SamsungModel).order_by(SamsungModel.no)
        ...
        return self.get_paginated_result('samsung/', data, request.args), 200


api.add_resource(Samsung, '/samsung')
